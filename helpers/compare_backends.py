...
from checked_capture import REQUIRED_PACKAGES, source_packages
...
CHECKED_ENVIRONMENT = {
    "FPGS_BENCH_ISAACLAB",
    "FPGS_BENCH_NEWTON",
    "FPGS_BENCH_RUN_SHA256",
    "FPGS_BENCH_LEGACY_CAPACITY",
}
SOLVER_CAPACITIES = {
...
ADDITIONAL_RECIPES = {
    "keyboard-so101": ("IsaacContrib-Keyboard-SO101", (), {}),
    "anymald-rough": ("Isaac-Velocity-Rough-AnymalD", (), {}),
    "g1-flat": ("Isaac-Velocity-Flat-G1", (), {}),
    "h1-flat": ("Isaac-Velocity-Flat-H1", (), {}),
    "h1-rough": ("Isaac-Velocity-Rough-H1", (), {}),
    "go2-flat": ("Isaac-Velocity-Flat-UnitreeGo2", (), {}),
    "go2-rough": ("Isaac-Velocity-Rough-UnitreeGo2", (), {}),
    "cassie-flat": ("Isaac-Velocity-Flat-Cassie", (), {}),
    "cassie-rough": ("Isaac-Velocity-Rough-Cassie", (), {}),
    "franka-reach": ("Isaac-Reach-Franka", (), {}),
    "ur10-reach": ("Isaac-Reach-UR10", (), {}),
    "franka-drawer": ("Isaac-Open-Drawer-Franka", (), {}),
}
...
def parse_flags(entries: list[str], gpus: list[int]) -> dict[int, dict[str, str]]:
    ...
    for entry in entries:
        ...
        if (
            ...
            or not equals
            or not re.fullmatch(r"(?:FEATHER_PGS_|NEWTON_NARROW_PHASE_|NEWTON_HEIGHTFIELD_)[A-Z0-9_]+", name)
            or "\0" in value
        ):
            raise ValueError(
...
import sys
from types import ModuleType
checker = ModuleType('_fpgs_preflight_imports')
checker.__file__ = sys.argv[3]
exec(compile(Path(checker.__file__).read_bytes(), checker.__file__, 'exec'), checker.__dict__)
selected = checker.select_imports(Path(sys.argv[1]), Path(sys.argv[2]))
with contextlib.redirect_stdout(sys.stderr):
    for name in checker.REQUIRED_PACKAGES:
        importlib.import_module(name)
provenance = checker.import_provenance(selected, required=checker.REQUIRED_PACKAGES)
versions = {}
...
print(json.dumps({'python': sys.version, 'python_executable': sys.executable, 'python_prefix': sys.prefix,
                  'newton_file': provenance['loaded']['newton']['file'], 'distributions': versions,
...
"""
    ...
    for label, root in roots.items():
        info = json.loads(
            subprocess.check_output(
                [
                    ...
                    str(isaaclab),
                    str(root),
                    str(Path(__file__).resolve().with_name("checked_capture.py")),
                ],
                cwd=isaaclab,
                env=clean_environment(
                    {"PYTHONPATH": source_pythonpath(isaaclab, root), "CUDA_VISIBLE_DEVICES": "", "GPU": ""},
                    isaaclab,
                ),
                text=True,
            )
        ...
        validate_import_provenance(info.get("import_provenance"), source_packages(isaaclab, root))
        result[label] = info
    return result


def source_pythonpath(isaaclab: Path, newton: Path) -> str:
    """Select every checkout package ahead of stale editable-install mappings."""
    packages = source_packages(isaaclab, newton)
    return os.pathsep.join(dict.fromkeys(str(Path(value["root"]).parent) for value in packages.values()))


def validate_import_provenance(provenance: dict, selected: dict) -> None:
    """Bind a child report to source paths and hashes independently in the parent."""
    if not isinstance(provenance, dict) or provenance.get("selected") != selected:
        raise RuntimeError("Capture import provenance does not match the selected checkouts")
    expected = {name: str(Path(value["root"]) / "__init__.py") for name, value in selected.items()}
    loaded = provenance.get("loaded")
    if provenance.get("resolved") != expected or not isinstance(loaded, dict) or set(REQUIRED_PACKAGES) - loaded.keys():
        raise RuntimeError("Capture import provenance lacks required selected imports")
    for name, record in loaded.items():
        package = name.partition(".")[0]
        if package not in selected or not isinstance(record, dict):
            raise RuntimeError(f"Capture imported an unselected module: {name}")
        root = Path(selected[package]["root"])
        filename, paths = record.get("file"), record.get("paths")
        if not isinstance(paths, list) or (filename is None and not paths):
            raise RuntimeError(f"Capture import has no source location: {name}")
        if any(not isinstance(path, str) or not Path(path).resolve().is_relative_to(root) for path in paths):
            raise RuntimeError(f"Capture import paths escaped selected source: {name}")
        if filename is not None and (
            not isinstance(filename, str)
            or not Path(filename).resolve().is_relative_to(root)
            or not Path(filename).is_file()
            or file_hashes([Path(filename)])[filename] != record.get("sha256")
        ):
            raise RuntimeError(f"Capture import source or hash differs: {name}")
        if name in REQUIRED_PACKAGES and filename != expected[name]:
            raise RuntimeError(f"Capture required import has the wrong source: {name}")
        owner = record.get("generated_by")
        if owner is not None and (
            not isinstance(owner, str)
            or not name.startswith(owner + ".")
            or not isinstance(loaded.get(owner), dict)
            or loaded[owner].get("file") != filename
        ):
            raise RuntimeError(f"Capture generated namespace lacks its source owner: {name}")
...
def check_overflow_result(run: dict, drivers: dict[str, str]) -> None:
    ...
    legacy = run["environment"].get("FPGS_BENCH_LEGACY_CAPACITY") == "1"
    statuses = {"checked", "not_applicable"}
    if legacy:
        statuses.add("legacy_warning_screened_boundary_snapshots")
        if report.get("legacy_capacity_mode") is not True:
            raise RuntimeError("Legacy capture lacks explicit diagnostic coverage disclosure")
        log_path = directory / "capture.log"
        warnings = [
            line
            for line in log_path.read_text(errors="replace").splitlines()
            if re.search(r"Warning:.*(?:buffer overflow|hashtable (?:fill ratio|insert failures))", line)
        ]
        run["legacy_collision_log_screen"] = {
            "path": str(log_path),
            "sha256": file_hashes([log_path])[str(log_path)],
            "warnings": warnings,
            "whole_run_no_drop_proof": False,
        }
        if warnings:
            raise RuntimeError(f"Legacy collision capacity warnings invalidate timing: {warnings[:4]}")
    ...
    if (
        ...
        or any(
            ...
            or entry["collision"].get("status") not in statuses
    ...
    selected = source_packages(
        Path(run["environment"]["FPGS_BENCH_ISAACLAB"]), Path(run["environment"]["FPGS_BENCH_NEWTON"])
    )
    before, after = report.get("import_provenance_before"), report.get("import_provenance_after")
    validate_import_provenance(before, selected)
    validate_import_provenance(after, selected)
    if any(
        after["loaded"].get(name) != record for name, record in before["loaded"].items() if "generated_by" not in record
    ):
        raise RuntimeError("Capture import provenance changed during execution")
    if fix:
    ...
    *,
    ...
    for gpu in devices:
        ...
        env.update(
            ...
            PYTHONPATH=source_pythonpath(args.isaaclab, roots[backend]),
...
def main(argv: list[str] | None = None) -> int:
    ...
    files = [
        ...
        Path(__file__).resolve().with_name("checked_capture.py"),
        Path(capture.__file__).resolve(),
        *(capture.HARNESS / name for name in ("nsys_run.sh", "run_profiled.py", "analyze_nsys.py")),
    ]
    if args.check_overflow:
        files.append(Path(__file__).resolve().with_name("nsys_checked.sh"))
