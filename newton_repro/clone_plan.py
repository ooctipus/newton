...
class SiteRequest:
    ...
    Attributes:
        ...
        body_pattern: Prototype-local body-label regex. ``None`` denotes a
            global world-origin site (or, when :attr:`per_world` is ``True``, a
            bodyless site placed in each cloned world's frame).
        xform: Site transform as ``(px, py, pz, qx, qy, qz, qw)``.
        per_world: When ``True``, a bodyless site is created in every cloned
            world's frame (mirrors ``cl_register_site(per_world=True)``);
            requires :attr:`body_pattern` to be ``None``.
    """
