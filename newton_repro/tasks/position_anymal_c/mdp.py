...
from tasks.position_anymal_c.kernels import (
...
from tasks.position_anymal_c.networks import ActuatorNetLSTM, CNNEncoderPolicy
...
class MDP:
    ...
    def _load_captured_buffers(self) -> None:
        ...
        with np.load(os.path.join(self.extras_dir, "initial_observations.npz")) as data:
            height_scan = data["height_scan"].reshape(data["height_scan"].shape[0], -1).astype(np.float32, copy=False)
