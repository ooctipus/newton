...
"""Standalone port of ``isaaclab.sim.utils.newton_model_utils``.

Mirrors the upstream Newton shape-color alignment so the repro cloner produces
the same viewer colors as the live :func:`isaaclab_newton.cloner.newton_replicate`
path, without importing Isaac Lab. The upstream ``Timer`` instrumentation and the
user-facing ``FutureWarning`` are intentionally dropped; the color logic is copied
verbatim. Source: ``source/isaaclab/isaaclab/sim/utils/newton_model_utils.py``.
"""
