...
# Render-acceleration structures the camera/renderer builds on the live model
# (e.g. ``bvh_shape_enabled``); not physics, and absent on a freshly rebuilt model.
_SKIP_MODEL_PREFIXES = ("bvh",)
...
def _public_data_attrs(obj, skip: set[str], skip_prefixes: tuple[str, ...] = ()) -> dict[str, object]:
    data = {}
    for name in dir(obj):
...
def assert_model_state_equal(expected_model, expected_state, actual_model, actual_state) -> None:
    ...
    _compare(
        "model",
        _public_data_attrs(expected_model, _SKIP_MODEL_ATTRS, _SKIP_MODEL_PREFIXES),
        _public_data_attrs(actual_model, _SKIP_MODEL_ATTRS, _SKIP_MODEL_PREFIXES),
        mismatches,
    )
    _compare(
        "state",
