...
class NewtonSim:
    ...
    def __init__(
        ...
        # Apply the captured live post-reset model state (friction / inertia / joint
        # actuator properties) BEFORE the solver compiles, so the MuJoCo solver reads
        # the real values instead of the cloner-hook-frozen defaults.
        if model_state:
            self._apply_model_state(model_state)
        self.model_state_applied = bool(model_state)
    ...
    def _apply_model_state(self, model_state: Mapping) -> None:
        """Copy captured model arrays (friction, inertia, joint actuator props) onto the
        rebuilt model. For subset replay (fewer envs than captured) the env-major prefix
        ``src[:dst.numel()]`` is the globals + first-N-envs slice."""
        import torch

        for name, value in model_state.items():
            arr = getattr(self.model, name, None)
            if arr is None:
                continue
            dst = wp.to_torch(arr)
            src = torch.as_tensor(value, device=dst.device).to(dst.dtype).reshape(-1)
            if src.numel() < dst.numel():
                raise RuntimeError(f"model_state['{name}'] has {src.numel()} < model {dst.numel()} entries")
            dst.copy_(src[: dst.numel()].reshape(dst.shape))
