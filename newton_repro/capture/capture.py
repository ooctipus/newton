...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Do not convex-hull meshes on rebuild; preserve per-shape approximation + SDF from the USD "
        "(required for SDF assets like the factory nut/bolt).",
    )
    parser.add_argument(
        "--verify_device",
        type=str,
...
def _extract_sim_cfg(env_cfg) -> dict:
    ...
    return {
        ...
        "solver": _plain(class_to_dict(solver_cfg)),
        "collision": _plain(class_to_dict(collision_cfg)) if collision_cfg is not None else {},
        "shape": _plain(class_to_dict(default_shape_cfg)) if default_shape_cfg is not None else {},
    }
...
def _capture_site_requests() -> tuple[SiteRequest, ...]:
    ...
    return tuple(
        ...
        for (body_pattern, _per_world, _xform_key), (label, xform) in NewtonManager._cl_pending_sites.items()
    )
...
def _make_capture_hook(env_cfg, args_cli: argparse.Namespace, agent_cfg, original_replicate):
    ...
    def _hook(
        ...
        if getattr(args_cli, "no_simplify_meshes", False):
            # Preserve per-shape approximation + SDF baked in the exported USD (factory nut/bolt have SDF;
            # the blanket convex_hull re-approximation replaces SDF meshes with hulls and crashes rebuild).
        ...
        export_bundle(args_cli.output_dir, stage, clone_plan, sim_cfg)
...
def _write_reset_state(out_dir: str, env) -> None:
    ...
    # Optional locomotion command/terrain state (anymal-style goal_point). Absent for factory-style tasks.
    try:
        ...
        logger.info("No goal_point command/terrain to capture (task has none): %s", exc)
    ...
    # Capture the LIVE post-reset model state (friction, inertia, actuator gains) the
    # EventManager + actuator setup write after the cloner hook; the rebuilt bundle model
    # is frozen before those, so the standalone applies these verbatim.
    try:
        from isaaclab_newton.physics.newton_manager import NewtonManager

        live_model = NewtonManager.get_model()
        model_state = {}
        for name in (
            "shape_material_mu",
            ...
            arr = getattr(live_model, name, None)
            if arr is not None:
                model_state[name] = _torch_to_numpy(arr)
        if model_state:
            np.savez(os.path.join(extras_dir, "model_state.npz"), **model_state)
            logger.info("Captured live post-reset model state: %s", sorted(model_state))
    except Exception as exc:  # noqa: BLE001
        logger.warning("Could not capture live model state: %s", exc)
