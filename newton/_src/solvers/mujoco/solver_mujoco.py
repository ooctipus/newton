...
class SolverMuJoCo(SolverBase, CouplingInterface):
    ...
    def __init__(
        ...
        *,
        ...
        # Independent of MJWarp's per-world overflow flags: external conversion
        # can truncate Newton's prefix before MJWarp observes its demand. Only
        # contact_capacity_status(clear=True) acknowledges this sticky history.
        self._contact_capacity_status = wp.zeros(1, dtype=wp.int32, device=self.device)
    ...
    def contact_capacity_status(self, *, clear: bool = False) -> dict[str, bool]:
        """Read sticky external Newton-to-MJWarp contact-prefix failures.

        ``negative_count`` indicates an invalid raw Newton contact count;
        ``source_contacts`` and ``mjwarp_contacts`` indicate demand exceeding
        the public Newton Contacts capacity or MJWarp's pooled ``naconmax``.
        The raw prefix is checked before accounting for contact filtering, so
        flags conservatively report truncation even if omitted contacts would
        subsequently have been rejected. No flag estimates required capacity.

        This status is independent of native MJWarp warning bits. Native
        MuJoCo collision and the MuJoCo-C backend do not use this converter;
        their converter flags remain false, not evidence about native capacity.
        Flags survive later clean conversions, contact generations, model
        notifications, solver resets, and CUDA graph replay.

        Call outside capture on the execution stream or after joining it. This
        copies one integer to the host, without additional per-step launches.

        Args:
            clear: Acknowledge history after reading and return its previous
                flags. This does not repair affected states or resize storage.
                Discard affected results and correct capacity before retrying.

        Raises:
            RuntimeError: If called during CUDA graph capture.
        """
        if self._contact_capacity_status.device.is_capturing:
            raise RuntimeError("Contact capacity status must be read outside CUDA graph capture")
        mask = int(self._contact_capacity_status.numpy()[0])
        result = {
            name: bool(mask & (1 << bit))
            for bit, name in enumerate(("negative_count", "source_contacts", "mjwarp_contacts"))
        }
        if clear:
            self._contact_capacity_status.zero_()
        return result

    def check_contact_capacity(self) -> None:
        """Raise outside capture if an external contact prefix was invalid or truncated.

        This does not clear history, abort a running device graph, or check
        native MJWarp collision/constraint buffers; check their status separately.
        """
        settings = {
            "negative_count": "negative rigid_contact_count",
            "source_contacts": "rigid_contact_max (public Newton Contacts storage)",
            "mjwarp_contacts": "nconmax (pooled MJWarp contact storage)",
        }
        failed = [settings[name] for name, flagged in self.contact_capacity_status().items() if flagged]
        if failed:
            raise RuntimeError("External contact capacity check failed: " + ", ".join(failed))
