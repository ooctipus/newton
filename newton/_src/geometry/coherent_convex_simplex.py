...
def create_solve_closest_distance(
    support_func: Any,
    _support_funcs: Any = None,
    *,
    _initial_simplex: Any = None,
    _record_vertex: Any = None,
    _final_simplex: Any = None,
):
    """
    ...
    Args:
        ...
        _initial_simplex: Private current-geometry warm-start callback. It must
            return feasible vertices, at most three active slots, and two
            local endpoint feature-ID vectors.
        _record_vertex: Private callback to update local endpoint feature-ID
            vectors from the newly returned support vertex.
        _final_simplex: Private callback to record a completed simplex and
            return feasible witness points. All callbacks are absent by default.
    ...
    callbacks = (_initial_simplex, _record_vertex, _final_simplex)
    if any(callback is not None for callback in callbacks) and not all(callback is not None for callback in callbacks):
        raise ValueError("Private GJK simplex callbacks must be provided together")
    ...
    def solve_closest_distance_core(
        ...
        v = center.BtoA
        if wp.static(_initial_simplex is not None):
            simplex_v, initial_mask, feature_a, feature_b = _initial_simplex(
                geom_a, geom_b, orientation_b, position_b, data_provider
            )
            initial_indices = wp.vec4i(0)
            initial_count = int(0)
            for i in range(4):
                if (initial_mask & (wp.uint32(1) << wp.uint32(i))) != wp.uint32(0):
                    initial_indices[initial_count] = i
                    initial_count += 1
            if initial_count == 1:
                i0 = initial_indices[0]
                v = simplex_v[2 * i0 + 1]
                simplex_usage_mask = wp.uint32(1) << wp.uint32(i0)
                simplex_barycentric[i0] = 1.0
            elif initial_count == 2:
                v, simplex_barycentric, simplex_usage_mask = closest_segment(
                    simplex_v, initial_indices[0], initial_indices[1]
                )
            elif initial_count == 3:
                v, simplex_barycentric, simplex_usage_mask = closest_triangle(
                    simplex_v, initial_indices[0], initial_indices[1], initial_indices[2]
                )
        ...
        while iter_count > 0:
            ...
            if dist_sq < COLLIDE_EPSILON * COLLIDE_EPSILON:
                ...
                if wp.static(_final_simplex is not None):
                    point_a, point_b = _final_simplex(
                        simplex_v, simplex_barycentric, simplex_usage_mask, data_provider, feature_a, feature_b
                    )
            ...
            if wp.static(_record_vertex is not None):
                feature_a, feature_b = _record_vertex(w, free_slot, feature_a, feature_b)
