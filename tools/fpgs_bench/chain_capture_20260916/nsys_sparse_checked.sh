...
# Unchanged retained G1 capture/export/analyzer command, with the adjacent
# source-pinned observer accepting the explicit chain factory keys.
set -euo pipefail
...
CUDA_VISIBLE_DEVICES=${GPU:-0} nsys profile --capture-range=cudaProfilerApi --capture-range-end=stop -t cuda-sw,nvtx \
  ...
  uv run python "$here/checked_sparse.py" --isaaclab "$repo" --newton "$newton" \
