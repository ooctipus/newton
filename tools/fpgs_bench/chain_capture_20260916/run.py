...
def selected_variants(module):
    ...
    variants = ModuleType("selected_fullwarp_variants")
...
def main():
    ...
    replacement = """            spectral_flag = run["environment"].get("FEATHER_PGS_SPARSE_SPECTRAL_TANGENTS")
            ...
            group_flag = run["environment"].get("FEATHER_PGS_SPARSE_FULLWARP_GROUP")
            if group_flag not in ("0", "1"):
                raise RuntimeError("Require explicit full-warp grouping on both arms")
            grouped = group_flag == "1"
            if grouped:
                if not spectral:
                    raise RuntimeError("Full-warp grouping requires spectral tangents")
                solve += "_w4"
            if kinetic.get("fullwarp_group") != {"requested": grouped, "observed": grouped, "check_pass": True}:
                raise RuntimeError("Missing actual full-warp owner observation")
...
"""
    ...
    module.FLAGS.add("FEATHER_PGS_SPARSE_FULLWARP_GROUP")
