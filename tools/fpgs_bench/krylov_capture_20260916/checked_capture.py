...
"""Append Krylov/chord owner identity at the original untimed boundaries."""
...
def snapshot(solver):
    ...
    value = os.environ.get("FEATHER_PGS_KRYLOV_CHORD")
    if value not in ("0", "1"):
        raise RuntimeError("Require explicit Krylov/chord on both arms")
    if os.environ.get("FEATHER_PGS_SPECTRAL_RESIDUAL") != "0":
    ...
    for kernel in kernels:
        if bool(getattr(kernel, "_fpgs_krylov_chord", False)) != wanted:
            raise RuntimeError("Requested Krylov/chord factory differs from actual owner")
        if kernel.key.endswith("_kc24") != wanted:
            raise RuntimeError("Unexpected Krylov/chord kernel suffix")
    fields = {
        "max_world_dofs": 18,
    ...
    if actual != fields or tuple(solver._ink_sizes) != (18, 0, 0, 0):
        raise RuntimeError("Krylov/chord owner changed original solver settings")
    return {
        "requested": wanted,
        "observed": wanted,
        "check_pass": True,
        ...
        "native_source_sha256": [
            hashlib.sha256(kernel._fpgs_krylov_chord_native.encode()).hexdigest() for kernel in kernels
        ]
        if wanted
        else [],
        "scope": "Actual factory and configuration; not dynamic row admission or physical convergence proof",
        "numerical_qualification": "Experimental cost screen; known CPU finite-budget component tradeoffs remain",
    }
...
def main():
    ...
    original = ModuleType("_krylov_original_checked")
    ...
    def combined(harness, report, save, get_manager, compat=None, compact=None):
        ...
        def observed(physics):
            ...
            try:
                ...
                entry["krylov_chord"] = snapshot(get_manager()._solver)
                return metadata
