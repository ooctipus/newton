...
"""CPU-only cause-corrected lookahead / separate Q and absolute-normal latch.
...
def normal_merit(impulse, residual, normal_rows):
    """Current state absolute normal/comp stream; no proposal or new operator."""
    normal = float(np.max(np.maximum(-residual[normal_rows], 0), initial=0))
    comp = float(np.max(np.abs(impulse[normal_rows] * residual[normal_rows]), initial=0))
    return max(normal, comp) / 3e-5
...
def solve(J, L, diagonal, rhs, types, parents, mu, vhat, *, iterations=24, incoming=None, early_stop=True):
    ...
    work = dict.fromkeys(
        (
            ...
            "normal_merit_row_visits",
            "normal_merit_reductions",
            "latch_q_increases",
            "latch_normal_increases",
        ),
        0,
    )
    work.update(
    ...
    def proposal(x, r, kind):
        ...
        work["normal_merit_row_visits"] += len(normals)
        work["normal_merit_reductions"] += 1
        return value[0], value[1], normal_merit(x, r, normals)
    ...
    relaxation, score, q, n_merit = 1.0, None, None, None
    if iterations:
        proposed, q, n_merit = proposal(impulse, residual, "initial_proposals")
    for iteration in range(iterations):
        ...
        next_proposed, next_q, next_n_merit = proposal(next_impulse, next_residual, "lookahead_proposals")
        trial_q, trial_n_merit = next_q, next_n_merit
        q_increased, normal_increased = next_q > q, next_n_merit > n_merit
        if relaxation == 1.0 and (q_increased or normal_increased):
            work["latch_q_increases"] += int(q_increased)
            work["latch_normal_increases"] += int(normal_increased)
            relaxation = 0.5
            next_impulse = 0.5 * (impulse + next_impulse)
            next_delta = 0.5 * (kinetic_delta + next_delta)
            next_residual = 0.5 * (residual + next_residual)
            next_proposed, next_q, next_n_merit = proposal(
                next_impulse, next_residual, "midpoint_replacement_proposals"
            )
            work["latch_step"] = iteration + 1
            work["midpoint_values"] += 2 * n + dofs
        old_q, old_n_merit = q, n_merit
        n_merit = next_n_merit
        ...
        work["curve"].append(
            {
                "sweep": iteration + 1,
                "current_q": old_q,
                "current_normal_merit": old_n_merit,
                "trial_normal_merit": trial_n_merit,
                "accepted_normal_merit": n_merit,
                "q_increased": bool(q_increased),
                "normal_increased": bool(normal_increased),
                "trial_q": trial_q,
                "accepted_q": q,
    ...
    work.update(
        ...
        final_q=q,
        final_normal_merit=n_merit,
        added_persistent_normal_merit_values=1,
