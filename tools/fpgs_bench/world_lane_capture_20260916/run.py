...
"""Reuse the fixed-import paired protocol with untimed world-lane ownership checks."""
...
FLAG = "FEATHER_PGS_WORLD_LANE_STATE"
...
def main():
    ...
    spec = importlib.util.spec_from_file_location("fixed_world_lane_adapter", ADAPTER)
    adapter = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(adapter)
    adapter.EXTRA.extend(HERE / name for name in ("run.py", "checked_world_lane.py", "nsys_checked.sh"))
    sys.path.insert(0, str(adapter.TOOLS))
    import compare_variants as variants  # noqa: PLC0415

    owner = variants.owner
    adapter.install(owner)
    old_batch, old_check = owner.make_batch, owner.check_overflow_result
    helper, shell = HERE / "checked_world_lane.py", HERE / "nsys_checked.sh"

    def make_batch(*args, **kwargs):
        batch = old_batch(*args, **kwargs)
        for run in batch:
            if run["task"] != "franka" or run["environment"].get(FLAG) not in ("0", "1"):
                raise ValueError("Require Franka and an explicit world-lane mode on both arms")
    ...
    def check(run, drivers):
        ...
        for entry in report["boundaries"]:
            ...
            if (
                ...
                raise RuntimeError("Missing or inconsistent world-lane boundary observation")
