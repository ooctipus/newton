...
- Add preallocated batched inverse dynamics with optional external body wrenches and public batched forward kinematics
- Add `--render-fps` to cap example rendering rate without changing simulation frame timing
- Add `DynamicsInverse.estimate_memory()` to report exact persistent storage for batched inverse-dynamics evaluators
